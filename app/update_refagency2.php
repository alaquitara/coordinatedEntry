...
$mysqli = new mysqli("oniddb.cws.oregonstate.edu","laquitaa-db","XXXXXXXXXXX","laquitaa-db");
...
<html>
    ...
    <body>
		...
		<div class="container">
			<?php
			if(!($stmt = $mysqli->prepare("UPDATE referral_agencies SET name=?, streetNumber=?, streetName=?, cid=? WHERE id=?"))){
		    	echo "<p id='message'>Prepare failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
		    }
		    if(!($stmt->bind_param("sisii",$_POST['name'],$_POST['streetNumber'],$_POST['streetName'],$_POST['city'], $_POST['id']))){
		    	echo "<p id='message'>Bind failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
		    }
		    if(!$stmt->execute()){
		    	echo "<p id='message'>Execute failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
		    } else {
		    	echo "<p id='message'>Updated " . $stmt->affected_rows . " row in referral_agencies.</p>";
		    }
			?>
		</div>
	</body>
