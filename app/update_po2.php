...
$mysqli = new mysqli("oniddb.cws.oregonstate.edu","laquitaa-db","XXXXXXXXXXXXX","laquitaa-db");
...
<html>
    ...
    <body>
		...
		<div class="container">
			<?php
				if(!($stmt = $mysqli->prepare("UPDATE project_organizations SET name=?, specialtyArea=?, fundingAmount=? WHERE id=?"))){
			    	echo "<p id='message'>Prepare failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
			    }

				$funding = (empty($_POST['fundingAmount'])?NULL:$_POST['fundingAmount']);

			    if(!($stmt->bind_param("ssii",$_POST['name'],$_POST['specialtyArea'],$funding, $_POST['id']))){
			    	echo "<p id='message'>Bind failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
			    }
			    if(!$stmt->execute()){
			    	echo "<p id='message'>Execute failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
			    } else {
			    	echo "<p id='message'>Updated " . $stmt->affected_rows . " row in project_organizations.</p>";
			    }
			?>
		</div>
	</body>
