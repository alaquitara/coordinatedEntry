...
<html>
    <head>
        ...
        <title>CS 340 Project: Delete Project Organization</title>
    ...
    <body>
		...
		<div class="container">
			<?php
				if(!($stmt = $mysqli->prepare("DELETE FROM project_organizations WHERE id=?"))){
			    	echo "<p id='message'>Prepare failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
			    }
			    if(!($stmt->bind_param("i",$_POST['delete']))){
			    	echo "<p id='message'>Bind failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
			    }
			    if(!$stmt->execute()){
			    	echo "<p id='message'>Execute failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
			    } else {
			    	echo "<p id='message'>Removed " . $stmt->affected_rows . " row from project_organizations.</p>";
			    }
			?>
		</div>
	</body>
