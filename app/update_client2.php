...
$mysqli = new mysqli("oniddb.cws.oregonstate.edu","laquitaa-db","XXXXXXXX","laquitaa-db");
...
<html>
    ...
    <body>
		...
		<div class="container">
			<?php
			if(!($stmt = $mysqli->prepare("UPDATE clients SET fname=?, lname=?, SSN=?, Sex=?, DOB=?, AgencyID=? WHERE id=?"))){
			...
			if(!($stmt->bind_param("sssssii",$_POST['fname'],$_POST['lname'],$ssn,$_POST['sex'],$dob, $_POST['refAgency'], $_POST['id']))){
				echo "<p id='message'>Bind failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
			}
			if(!$stmt->execute()){
				echo "<p id='message'>Execute failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
			} else {
				echo "<p id='message'>Updated " . $stmt->affected_rows . " row in clients.</p>";
			}
			?>
		</div>
	</body>
