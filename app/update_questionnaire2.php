...
$mysqli = new mysqli("oniddb.cws.oregonstate.edu","laquitaa-db","XXXXXXXXXXXXXXX","laquitaa-db");
...
<html>
    ...
    <body>
		...
		<div class="container">
			<?php
				if(!($stmt = $mysqli->prepare("UPDATE questionnaire SET score=? WHERE id=?"))){
			    	echo "<p id='message'>Prepare failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
			    }
			    if(!($stmt->bind_param("ii",$_POST['score'], $_POST['id']))){
			    	echo "<p id='message'>Bind failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
			    }
			    if(!$stmt->execute()){
			    	echo "<p id='message'>Execute failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
			    } else {
			    	echo "<p id='message'>Updated " . $stmt->affected_rows . " row in questionnaire.</p>";
			    }
			?>
		</div>
	</body>
