...
<html>
    <head>
        ...
        <title>CS 340 Project: Delete Referral</title>
    ...
    <body>
		...
		<div class="container">
			<?php
				if(!($stmt = $mysqli->prepare("DELETE FROM referrals WHERE cid=? AND oid=?"))){
					echo "<p id='message'>Prepare failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
				}
				if(!($stmt->bind_param("ii",$_POST['cid'],$_POST['oid']))){
					echo "<p id='message'>Bind failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
				}
				if(!$stmt->execute()){
					echo "<p id='message'>Execute failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
				} else {
					echo "<p id='message'>Removed " . $stmt->affected_rows . " row from referrals.</p>";
				}
			?>
		</div>
	</body>
