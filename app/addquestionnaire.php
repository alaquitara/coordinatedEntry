...
<html>
    <head>
        ...
        <title>CS 340 Project: Add Questionnaire</title>
    ...
    <body>
		...
		<div class="container">
			<?php
				if(!($stmt = $mysqli->prepare("INSERT INTO questionnaire(score, CID) VALUES (?, ?)"))){
				echo "<p id='message'>Prepare failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
				}
				if(!($stmt->bind_param("ii",$_POST['score'], $_POST['client']))){
				echo "<p id='message'>Bind failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
				}
				if(!$stmt->execute()){
				echo "<p id='message'>Execute failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
				} else {
				echo "<p id='message'>Added " . $stmt->affected_rows . " row to questionnaire.</p>";
				}
			?>
		</div>
	</body>
