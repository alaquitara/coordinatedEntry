...
<html>
    <head>
        ...
        <title>CS 340 Project: Add Project Organizations</title>
    ...
    <body>
		...
		<div class="container">
			<?php
				if(!($stmt = $mysqli->prepare("INSERT INTO project_organizations(name, specialtyArea, fundingAmount) VALUES (?,?,?)"))){
					echo "<p id='message'>Prepare failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
				}

				//If value is empty ensure NULL is entered and not an empty string
				$funding = (empty($_POST['fundingAmount'])?NULL:$_POST['fundingAmount']);

				if(!($stmt->bind_param("ssi",$_POST['name'],$_POST['specialtyArea'],$funding))){
					echo "<p id='message'>Bind failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
				}
				if(!$stmt->execute()){
					echo "<p id='message'>Execute failed: "  . $stmt->errno . " " . $stmt->error . "</p>";
				} else {
					echo "<p id='message'>Added " . $stmt->affected_rows . " row to Project Organizations.</p>";
				}
			?>
		</div>
	</body>
