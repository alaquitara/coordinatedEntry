...
$mysqli = new mysqli("oniddb.cws.oregonstate.edu","laquitaa-db","XXXXXXXXXX,"laquitaa-db");
...
<html>
    <head>
        ...
        <title>CS 340 Project: Delete Referral Agency</title>
    ...
    <body>
		...
		<div class="container">
			<?php
			if(!($stmt = $mysqli->prepare("DELETE FROM referral_agencies WHERE id=?"))){
			...
			if(!$stmt->execute()){
				...
				echo "<p id='message'>Removed " . $stmt->affected_rows . " row from referral_agencies.</p>";
			}
			?>
		</div>
	</body>
